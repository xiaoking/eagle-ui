...
export default class ValidatorPanel extends Component {
    ...
    setTipsOffset(element){
        let node = null,
            name = element.name,
            tip =React.findDOMNode(this.refs[this.formName]),
    ...
    getElements(){
        return React.findDOMNode(this.refs[this.formName]).querySelectorAll(`[data-validate]`);
    }
    ...
    render() {
        ...
        return (
            <Component {...this.props} ref={this.formName} className={this.getClassName('validate')}>
                {this.props.children}
                <div className={classnames(
                            this.getClassName(`validate-tips`),
                            `${this.init ? "":this.state.message ? "fadein" :"fadeout"}`
                        )}>
                    <i></i>
                    {this.state.message}
                </div>
